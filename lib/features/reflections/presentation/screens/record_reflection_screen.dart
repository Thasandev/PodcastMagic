        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const Spacer(),
                    Text('Record Reflection', style: AppTextStyles.overline.copyWith(color: AppColors.grey500)),
                    const Spacer(),
              ...
              Text(
                _isRecording ? 'Recording...' : (_hasRecording ? 'Recording saved' : 'Tap to start'),
              ...
              if (_hasRecording) ...[
                ...
                Row(
                  ...
                  children: [
                    _VisibilityChip(label: '🌍 Public', isSelected: _visibility == 'public', onTap: () => setState(() => _visibility = 'public')),
                    const SizedBox(width: 12),
                    _VisibilityChip(label: '👥 Friends Only', isSelected: _visibility == 'friends', onTap: () => setState(() => _visibility = 'friends')),
                  ],
                ),
                        ...
                        child: KGradientButton(
                          ...
                          onPressed: () {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Reflection shared! 🎙️'), backgroundColor: AppColors.darkCard),
                            );
                            context.pop();
                          },
                        ),
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: (_isRecording ? AppColors.error : AppColors.primary)
                                  .withValues(alpha: _isRecording ? 0.3 + _pulseController.value * 0.2 : 0.3),
                              blurRadius: _isRecording ? 20 + _pulseController.value * 10 : 20,
                            ),
                          ],
                        ),
                        child: Icon(
      ...
      children: [
        ...
        ...List.generate(3, (i) {
          return AnimatedBuilder(
            animation: _pulseController,
            builder: (context, _) {
              final scale = 1.0 + i * 0.25 + (_isRecording ? _pulseController.value * 0.1 : 0);
              return Container(
                width: 160 * scale,
                height: 160 * scale,
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  border: Border.all(
                    color: AppColors.primary.withValues(alpha: _isRecording ? 0.25 - i * 0.07 : 0.08 - i * 0.02),
                    width: 1,
                  ),
                ),
              );
...
class _VisibilityChip extends StatelessWidget {
  ...
  const _VisibilityChip({required this.label, required this.isSelected, required this.onTap});
